...
# chars; returns 1 when the file is missing or not parseable. `resolved` is
# compared verbatim: a git+ssh:// vs ssh:// difference is a real lockfile
# mismatch that a correct install fixes, never something to paper over.
nm_lockfile_hash() {
  local f="$1" out
  [[ -f "$f" ]] || return 1
