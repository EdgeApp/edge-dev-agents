...
try:
    ...
    check('completion directive releases AND writes a one-event judge waiver', (not held()) and os.path.exists(WAIVER) and 'operator-directed complete' in open(WAIVER).read())
    check('completion directive text says the judge is waived for this completion and applies again after',
          'WAIVED' in out and 'next completion event only' in out and 'applies again' in out and 'normal gates' not in out, out[:400])
    clear()
    out = prompt('Complete the run, I will publish tomorrow')
    check('the waiver carries the operator words that triggered it', 'I will publish tomorrow' in open(WAIVER).read(), open(WAIVER).read()[:120])
    clear()
    out = prompt('looks good, go ahead')
    check('a plain release claims no waiver and promises the normal gates', (not os.path.exists(WAIVER)) and 'normal gates' in out and 'WAIVED' not in out, out[:200])
    out = prompt('bypass the completion judge')
    check('a bare bypass grant writes the waiver and says it covers one event',
          os.path.exists(WAIVER) and 'operator-directed bypass' in open(WAIVER).read() and 'NEXT completion event only' in out, out[:200])
    clear()
    ...
    check('stop text says the judge is waived for that one write only', 'waived for that one write' in out and 'applies again' in out, out[:400])
    clear()
