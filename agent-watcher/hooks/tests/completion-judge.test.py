...
try:
    ...
    marker([])
    collect(); check('collector: followup with no comments names the re-arm signals as the ask', '(no operator comments in scope)' in open(EVID).read() and 'field deltas and GitHub counters' in open(EVID).read())
    marker([ASK])
