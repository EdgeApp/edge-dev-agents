...
| `block-broad-process-kill.sh` | PreToolUse / Bash (all sessions) | Blocks every `pkill`, `killall`, and `pgrep`-fed kill; only `kill <pid>` passes | Box-wide pattern kills: task claudes carry maestro-mcp.json in argv, so `pkill -f maestro` kills every agent |
