...
| `require-block-validation.sh` | Genuinely unregistered. A full PreToolUse gate on `update-status.sh --blocked yes` against a `/tmp/agent-blocker-verdict-<gid>.json` verdict. Nothing execs it, and the block path it guarded is now the completion judge's `block` event. `~/.cursor/README.md` already calls it "an unregistered legacy kept for history", but `~/.cursor/skills/blocker-validator/SKILL.md` line 56 still describes it as the live gate, which is stale. |
