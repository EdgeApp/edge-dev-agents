...
| `refresh-main-checkouts.sh` | keep the ~/git main checkouts (the node_modules clone SOURCES) current: fast-forward the default branch + `npm ci` on lockfile change, ONLY when clean and on the default branch (dirty/feature/detached checkouts are reported and left alone). Scheduled: launchd daily 04:15 + every 30min idle sweep (both `--require-idle`: skip while any run session or in_use pool sim exists; idle job throttled to one sweep per 6h) |
...
| `orch-tui.js` | read-only orchestration dashboard (`orch-tui` alias), NO controls: vitals (load vs guardrail max with SPAWNS GATED flag, available RAM via the watcher's vm_stat formula, cap, watcher/watchdog tick age), launchd job health (`com.jontz.*` last exit codes; config-watch rc 1 renders as DRIFT), Asana `agent_status` tally for the project, the session-tui fleet model, slots ⨯ liveness (STALE flag = allocated slot with no session) + per-slot Metro LISTEN check, sim pool ⨯ liveness, worktrees ⨯ liveness, and a merged interesting-lines tail of the watcher+watchdog logs. Local state refreshes every 10s, the Asana tally every 60s. `--dump` prints the model as JSON |
