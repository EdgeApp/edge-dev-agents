...
2. **Downgrade-specific** — if this is a downgrade, also test it against `<downgrade-fallbacks>`. A documented gotcha with a written continue-workaround (the Fabric SIGSEGV is the canonical one), a weaker-way verification that skipped the prescribed drive, or a pre-attempt bail → DENY with the documented workaround / genuine attempt as `what_to_try`. A playbook-sanctioned fallback → ALLOW only with a corroborating genuine-attempt entry AND no remaining workaround (step 4 below).
3. **Visual-surface check** — if the reason concerns a change a USER SEES (a row, badge, spinner, empty state, copy, layout), the pixels are corroborating evidence like the attempt-log is for funds. List the run's frames and look for a hack-forced one:
```bash
ls /tmp/agent-proof-<gid>-*.png 2>/dev/null
```
A `*-HACKED-*.png` covering the surface → the visual bar is met (judge the rest of the reason normally). No frame at all → DENY per the visual bullets above, with the hack-verify recipe as `what_to_try`.
4. If it matches an **allow** category that requires corroboration (observed loss, provider/geo block, or a sanctioned downgrade-fallback) → read the attempt-log; ALLOW only if a matching `loss:`/`failed:<provider>`/`blocked:` entry exists for a real attempt. No corroborating entry → DENY ("attempt it first and log the result").
5. If it matches a slot-unsatisfiable **allow** category (credential, destructive-no-recovery, true ambiguity) → ALLOW.
6. Anything unclear or not cleanly in a category → DENY (default-deny), and say what attempt would clarify it.
</step>
