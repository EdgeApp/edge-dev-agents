...
| A22 | tested-field-accuracy | the task's `tested` multi-select matches the run's actual evidence: `Simulator` only with a genuine pixel-verified in-app drive (open the proofs; springboard/wrong-scene frames invalidate), `Unit Tests` only when a test suite actually executed (zero-tests-found runs, tsc/eslint alone do NOT count), `CouchDB` for couch-backed tests, `Untested` exclusive and set when nothing ran, field non-empty | `Simulator` credited on a quote-stop or wrong-device proof; `Unit Tests` credited on static checks alone; field left empty on a run that should have set it | one-shot step 7 set-tested rule; `~/.config/agent-watcher/set-tested.sh` options — **NA for runs predating the tested field (2026-06-12)** |
