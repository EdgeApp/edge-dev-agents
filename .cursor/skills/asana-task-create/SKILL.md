...
<rule id="no-unrequested-comms">Never write a step into the description that has the implementing agent message anyone (reply in a Slack thread, post to a channel, DM, email, ask or report back to the reporter or a partner) unless the operator explicitly asked for that step in this conversation. A source thread or reporter in the context is NOT authorization: link it as context only. The script refuses notes that direct comms; pass `--comms-authorized "<the operator's words>"` only when you can quote the operator asking for it, never to get past the gate.</rule>
