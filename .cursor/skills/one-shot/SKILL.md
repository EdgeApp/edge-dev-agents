...
**Editing an EdgeApp gui dependency (edge-core-js, edge-currency-accountbased, edge-exchange-plugins, edge-currency-plugins, edge-login-ui-rn, …).** If the task changes a dependency repo (with or without `edge-react-gui` changes), create a co-located worktree for the dependency AND `edge-react-gui` — siblings under the same `<task-gid>/` dir, so `updot` can find the *modified* dep. The actual in-app **linking + test mechanism is owned by `/build-and-test`'s `gui-dependency-integration`** (DEBUG_ flag + the dep's dev-server for webview plugins, `updot` for core/native deps, `YOLO_` auto-login for login) — don't duplicate or hardcode it here. Run each repo's scripts with that repo's package manager (yarn or npm per its lockfile — **yarn is being phased out toward npm; check the lockfile, don't assume**).
</step>
