...
<principle id="no-incident-narration">Rules carry the imperative, never the incident that motivated it. No audit citations ("11/13 runs skipped this"), no dated rationalization tags ("the X run, 2026-06-11"), no history of how a behavior used to work ("this flipped from off-by-default"). The evidence trail lives in eval reports and git history; a rule that narrates its origin grows forever and dilutes the file (the documented failure mode: "if Claude keeps violating a rule, the file is probably too long"). When patching a rule after an incident, write the counter-imperative the incident taught — not the incident.</principle>
<principle id="rationale-once">A rationale is stated ONCE, in the rule that owns it; every other rule that depends on it cross-references the id ("per `never-self-respawn`") without restating the why. Restated rationale is the main way related rules balloon in lockstep.</principle>
