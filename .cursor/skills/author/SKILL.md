...
<principle id="name-tracks-scope">Every NAME an agent or another file can cite must keep describing what it actually does. This covers skills, scripts, rule ids, step ids, flags, and any other identifier class — the list is OPEN, so apply it to whatever kind of name a change touches rather than checking membership in it. Names drift as responsibilities accrete — a thing named for one purpose quietly grows three. When an edit broadens, narrows, or shifts scope, treat the name as part of the diff: if it no longer fits, the rename belongs in THIS change. Scale the reference sweep to how a stale name fails: a skill or script name fails CLOSED (`command not found` at the call site, or a hook that never fires), while a rule or step id fails OPEN — `per <old-id>` still reads like a valid citation and resolves to nothing, so a sweep is the only thing that surfaces it. Never rename silently (renames ripple through callers, launchd jobs, hook registrations, docs) — propose the clearer name and get confirmation. Operationalized in `<revision-checklist>`; see also `<companion-scripts>`.</principle>
...
4. **Check dependent callers before any script/command change**: Before adding, updating, renaming, or removing any citable identifier — command, skill, script, rule id, step id, flag, output contract, or any other name something else can reference (`name-tracks-scope` owns the open-list rationale) — search for direct callers/references and update them in the same change.
...
12. **Re-check name accuracy after scope changes — and prompt before renaming**: After editing, assess every name the change touched — the skill, its scripts, and any rule or step id whose scope moved — and decide whether each still accurately and clearly describes what it does (per the `name-tracks-scope` principle). The tell: you can no longer state what it does in one phrase that matches its name, or its summary needs "and also…". If a name has drifted:
    ...
    - **On confirmation, rename safely** following item 4's dependent-caller discipline. Cover every surface: the file itself; every textual reference (`rg -n "<old-name>" ~/.cursor ~/.config/agent-watcher ~/.claude/settings.json`, whose roots must include `~/.claude/settings.json` — hook `command` paths live ONLY there, outside the other two roots, and a stale one surfaces only when the hook next fires); any launchd registration (the plist filename, its `Label`, `ProgramArguments` path, and `Standard{Out,Error}Path`, plus `launchctl bootout <old>` then `bootstrap`/`kickstart <new>`); for a rule or step id, the eval rubrics and `rubric-drift.lock.json` anchors that cite it as `<skill>:<id>`; docs/READMEs; and the distribution copy in the synced repo (use `git mv` to preserve history). List every surface updated in your final response.
</revision-checklist>
