...
<rule id="commit-via-script">Fixups MUST use `~/.cursor/skills/lint-commit.sh --fixup <target-sha> --for auto -m "<what changed and which finding it answers>" [files...]` (body and kind required; bugbot findings are always `--for auto`, so they condense apart from fixups answering a person). The script asks the review-mode oracle whether to fold (preserve leaves the fixup at HEAD for slotting). Do not run `git commit` directly and do not manually run eslint — the commit script handles it.</rule>
...
4. Commit as a fixup, body required:
   ```bash
   ~/.cursor/skills/lint-commit.sh --fixup <target-sha> --for auto -m "<what changed and which finding it answers>" <files...>
   ```
