...
<rule id="commit-via-script">Fixups MUST use `~/.cursor/skills/lint-commit.sh --fixup <target-sha> -m "<what changed and which finding it answers>" [files...]` (body required; a bodyless fixup is bounced). The script asks the review-mode oracle whether to fold (preserve leaves the fixup at HEAD for slotting). Do not run `git commit` directly and do not manually run eslint — the commit script handles it.</rule>
...
<rule id="fixup-target-headline">Before each fixup, run `git log --oneline -- <changed-file>` to find the commit that introduced the behavior being fixed and target its sha. The fixup must target a real commit on the branch so the later autosquash resolves correctly.</rule>
...
2. Locate the fixup target:
   ...
   ```
   Pick the commit that introduced the behavior being fixed; note its sha.
...
4. Commit as a fixup, body required:
   ```bash
   ~/.cursor/skills/lint-commit.sh --fixup <target-sha> -m "<what changed and which finding it answers>" <files...>
   ```
...
After every valid thread has been committed and slotted, condense same-target fixups (one per target, so the SHAs Step 4e cites survive Step 4f's finalize), then push:
