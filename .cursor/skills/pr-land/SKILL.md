...
| `pr-land-automerge.sh` | Arm GitHub auto-merge (or turn it off with `--disarm`), gated on reviewer bots being done and bot threads clear (see `bot-thread-gate`) |
...
| `npm-publish-web.sh` | Login-if-needed + publish via npm web/link auth under a PTY; emits `AUTH_URL` and `AUTH_DONE` lines to relay |
