...
<rules>
<rule id="default-branch-only">Everything happens on the repo default branch (`main`). The script checks it out itself before any diff or commit and refuses to switch away from uncommitted work; no per-machine or per-user branch exists any more, and no flag commits elsewhere.</rule>
