...
- **"no destination wallet" / "the receiving account has no X wallet" / "would need to create a wallet to send to"** → a missing receive-side wallet is created, never conceded (build-and-test `create-missing-destination-wallet`): add it in the account the send targets (Add Wallet, or Add / Edit Tokens for a token) and continue. It needs no funds. DENY. Try: create the destination wallet, then drive the send.
...
- **User-only credential** — 2FA, account password, OAuth re-auth, signing-key passphrase the slot does not hold (NOT a roster account whose password and OTP key are stored in `~/.config/edge-secrets/`: read them from there). ALLOW (no attempt-log needed; it is categorically slot-unsatisfiable).
