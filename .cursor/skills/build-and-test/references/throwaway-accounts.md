...
| `agent-tw-1159917970a` | `Agent3Throwaway1` | `1234` | agent-sim-pool-3 | 2026-08-27 | BTC, ETH, LTC, BCH, DASH (app defaults) | Empty. Created as `agent-tw-1159917970`, renamed to `...a` by the password-reminder test. Password reminder counters are used up (passwordUseCount 1). |
| `agent-tw-pwrec-8827` | `Agent3Throwaway1` | `1234` | agent-sim-pool-0 | 2026-08-27 | DASH only ("My Dash") | Empty (funded to $24 in DASH for the password-recovery-milestone test, then swept back to edge-funds). Region unset. `passwordRecoveryRemindersShown['20']` is now TRUE and the Send-scene scam-warning acknowledgement is accepted, so it is NOT a clean slate for a re-test of the $20 milestone — create a fresh one for that. |
| `agent-tw-pwrec-901` | `Agent3Throwaway1` | `1234` | agent-sim-pool-2 | 2026-09-01 | BTC, ETH, LTC, BCH, DASH (app defaults) | Empty (funded to $24.05 in DASH for the password-recovery-milestone re-test, then swept back to edge-funds). `passwordRecoveryRemindersShown['20']` is now TRUE and the Send-scene scam-warning acknowledgement is accepted, so it is NOT a clean slate for a re-test of the $20 milestone. Region unset. |
| `agent-tw-pwrec-903` | `Agent3Throwaway1` | `1234` | agent-sim-pool-1 | 2026-09-03 | BTC, ETH, LTC, BCH, DASH (app defaults) | Empty (funded to $23.00 in DASH for the password-recovery per-account-grace test, then swept back to edge-funds). `passwordRecoveryRemindersShown['20']` is now TRUE and the Send-scene scam-warning acknowledgement is accepted, so it is NOT a clean slate for a re-test of the $20 milestone. Region unset. |
