...
2. **Link the MODIFIED dep — pick the mechanism that matches the dep, via the gui worktree's `env.json`.** Run EACH repo's scripts with THAT repo's package manager — detect by lockfile: `yarn.lock`→yarn (`yarn <script> [args]`), `package-lock.json`→npm (`npm run <script> [-- args]`, or `npm start`). **yarn is being phased out toward npm across Edge repos — never assume yarn; check the lockfile each time** (a repo may be yarn today and npm tomorrow).
   - **Webview-plugin deps → `DEBUG_*` flag + the dep's live dev-server (preferred; no gui rebuild).** (a) Set the flag `true` in the GUI worktree's `env.json`; (b) in the DEP worktree, start its dev-server (webpack serve) backgrounded and left running for the whole maestro test — `yarn start` OR `npm start` per the dep's lockfile. The app's webview then loads the local bundle from the host's localhost (the sim reaches host localhost directly). Map: `edge-currency-accountbased`→`DEBUG_ACCOUNTBASED` (:8082), `edge-exchange-plugins`→`DEBUG_EXCHANGES` (:8083), `edge-currency-plugins`→`DEBUG_CURRENCY_PLUGINS` (:8084), in-app plugin bundle→`DEBUG_PLUGINS` (:8101). If a DEBUG_ run is flaky (webview can't reach the dev-server, ATS/cleartext blocks it, or a recompile races init), fall back to `updot` for that dep.
   - **`edge-core-js` → `updot`, NOT `DEBUG_CORE`.** `DEBUG_CORE` makes the core WebView load its source from hardcoded `http://localhost:8080/` (`edge-core-js/src/io/react-native/react-native-webview.tsx`: `source={debug ? 'http://localhost:8080/' : null}`) — a hard runtime dependency that races core init, is cleartext-HTTP (ATS-sensitive), and when it hiccups the ENTIRE app fails (core is the foundation, not a contained plugin). That's the long-standing "DEBUG_CORE is buggy". For headless/automated runs, updot core into the gui then prepare, via the GUI's package manager — yarn: `yarn updot edge-core-js && yarn prepare && yarn prepare.ios`; npm: `npm run updot -- edge-core-js && npm run prepare && npm run prepare.ios`. Bakes local core statically into `node_modules`, no dev-server, no race. (`DEBUG_CORE` stays fine for interactive live-reload dev.)
   - **Other metro/native deps → `updot` (build-time bake; no dev-server exists).** For `edge-login-ui-rn`, `edge-currency-monero`, `react-native-piratechain`/`-zcash`/`-zano`: updot the dep then the gui's `prepare`, via the GUI's package manager — yarn: `yarn updot <dep> && yarn prepare`; npm: `npm run updot -- <dep> && npm run prepare` (add `prepare.ios` for native-module deps) — then rebuild.
3. **Login:** the test account auto-logs-in via the `YOLO_*` env knobs (already set: `YOLO_USERNAME=roster-qa-b`, `YOLO_PIN=1111`, consumed in `LoginScene.tsx`). Keep them set so the maestro run reaches the logged-in app; when the change is to `edge-login-ui-rn` specifically, these are the lever for exercising the login flow — adjust only if the change requires driving the login UI differently.
...
There can be multiple "iPhone 16 Pro Max" devices across runtimes. **Only the iOS 18 device holds the test account** (`roster-qa-b`, PIN `1111`, region California/USA, BTC wallet). The iOS 26.x device does NOT.
...
<case name="Repo is edge-react-gui but the test account / sim was wiped">Set `blocked = Yes` — the test relies on `roster-qa-b` with PIN 1111. Re-provisioning is a human step.</case>
</edge-cases>
