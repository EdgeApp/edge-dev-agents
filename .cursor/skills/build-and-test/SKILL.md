...
3. **Login:** the test account auto-logs-in via the `YOLO_*` env knobs (set by workspace init: `YOLO_USERNAME=roster-primary`, `YOLO_PIN=0000`, consumed in `LoginScene.tsx` — pinned by `setup-task-workspace.sh` on every worktree's env.json copy). Keep them set so the maestro run reaches the logged-in app; when the change is to `edge-login-ui-rn` specifically, these are the lever for exercising the login flow — adjust only if the change requires driving the login UI differently.
...
There can be multiple "iPhone 16 Pro Max" devices across runtimes. **Only the iOS 18 device holds the test accounts** (the `funded-test-accounts` roster; default login `roster-primary` PIN `0000`). The iOS 26.x device does NOT.
...
<case name="Repo is edge-react-gui but the test account / sim was wiped">Set `blocked = Yes` — the test relies on the roster accounts (default `roster-primary`, PIN 0000) being present on the sim image. Re-provisioning is a human step.</case>
</edge-cases>
