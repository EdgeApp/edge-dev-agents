...
2. **Link the MODIFIED dep — pick the mechanism that matches the dep, via the gui worktree's `env.json`:**
   - **Webview-plugin deps → `DEBUG_*` flag + the dep's live dev-server (preferred; no gui rebuild).** (a) Set the flag `true` in the GUI worktree's `env.json`; (b) in the DEP worktree, start its dev-server — `yarn start` (webpack serve), backgrounded, left running for the whole maestro test. The app's webview then loads the local bundle from the host's localhost (the sim reaches host localhost directly). Map: `edge-currency-accountbased`→`DEBUG_ACCOUNTBASED` (:8082), `edge-exchange-plugins`→`DEBUG_EXCHANGES` (:8083), `edge-currency-plugins`→`DEBUG_CURRENCY_PLUGINS` (:8084), in-app plugin bundle→`DEBUG_PLUGINS` (:8101). If a DEBUG_ run is flaky (webview can't reach the dev-server, ATS/cleartext blocks it, or a recompile races init), fall back to `updot` for that dep.
   - **`edge-core-js` → `updot`, NOT `DEBUG_CORE`.** `DEBUG_CORE` makes the core WebView load its source from hardcoded `http://localhost:8080/` (`edge-core-js/src/io/react-native/react-native-webview.tsx`: `source={debug ? 'http://localhost:8080/' : null}`) — a hard runtime dependency that races core init, is cleartext-HTTP (ATS-sensitive), and when it hiccups the ENTIRE app fails (core is the foundation, not a contained plugin). That's the long-standing "DEBUG_CORE is buggy". For headless/automated runs use `updot edge-core-js && yarn prepare && yarn prepare.ios` — bakes local core statically into `node_modules`, no dev-server, no race. (`DEBUG_CORE` stays fine for interactive live-reload dev.)
   - **Other metro/native deps → `updot` (build-time bake; no dev-server exists).** For `edge-login-ui-rn`, `edge-currency-monero`, `react-native-piratechain`/`-zcash`/`-zano`: `updot <dep>` then the gui's `prepare` (`yarn updot <dep> && yarn prepare`, following the gui PM; `yarn prepare.ios` for native-module deps), then rebuild.
