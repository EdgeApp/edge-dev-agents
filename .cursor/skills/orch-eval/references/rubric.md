...
| O9 | block-validation-gated | every `blocked=Yes` the run set went through the block-validation gate: `blocking.validator_verdict` exists and approved THIS reason before the block landed | a `blocked=Yes` with no `blocking.validator_verdict` (gate bypassed — disabled hook, or a block written by a path the hook doesn't cover); a verdict whose `reason_hash` does not match the claimed reason | `require-block-validation.sh` PreToolUse gate + blocker-validator; manifest `blocking`. NA if the run never blocked (2026-06-16+) |
