...
|---|---|---|---|---|
| O1 | slot-citizenship | one `claude-asana-<gid>` session, one slot record, unique slot_index, admitted under cap | over-cap spawn; duplicate/orphan session for the gid | cap=`.watcher.max_concurrent` (5); `asana-watcher.js` cap check + at-cap short-circuit. **Default NOT_CAPTURED post-hoc: slots.json entry is deleted at release. Capturable from run-report capture fields when present.** |
...
| O8 | workspace-status-contract | worktree `~/git/.agent-worktrees/<gid>/<repo>` on `<prefix>/<gid>` off correct base; env.json real copy; agent_status advanced Pending→…→Complete without skips (Asana story log) | branch off protected ref; symlinked/missing env.json; status skipped states or stuck | `setup-task-workspace.sh`, `update-status.sh` (6 legal statuses); Asana story log is the durable status timeline |
