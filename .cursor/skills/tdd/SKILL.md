...
<rule id="repo-separation">When the design spans repos, each repo gets its own detailed-design SUBSECTION of Design overview (`### N.1 Detailed design: <repo>`, `### N.2 …`), whose own internals sit one level below (`#### N.1.1 …`). Design overview itself owns the repo table (repo, deliverable PR link once it exists, scope pointer) and the cross-repo diagram, so the per-repo detail reads as the elaboration of one design with a seam rather than as several designs sharing a file. Sibling top-level sections are wrong here: any later section numbered between them (Testing is the one that happens) then reads as a peer of the repos and splits the design in half. Never interleave two repos' changes in one subsection; where the repos interact, each anchor-links the other side of the seam and the cross-repo diagram (`diagrams-and-signatures`). Depth is the limit to watch: when a repo's detail needs a fifth heading level, that repo's design wants its own doc, linked from the repo table.</rule>
<rule id="clickable-everything">The doc has a `## Contents` ToC and every in-body reference to a section, decision, or test case is a markdown anchor link, GFM slugs (lowercase, punctuation stripped, spaces to hyphens). One ToC entry per `##` section, PLUS a nested entry per repo subsection on a multi-repo doc (`repo-separation`) — nesting the repos under Design overview otherwise drops them out of the contents, and a reader who came for one repo has nothing to click. Renumbering is where this rule breaks: moving a section rewrites both the anchor and the number a link DISPLAYS, and a link whose text says `7.1` while its anchor resolves to `4.1` still resolves, so it reads as correct and is not. `tdd-lint.sh` verifies ToC resolution, flags unlinked "section N"/"decision N" text, and compares every link's displayed number against the heading its anchor resolves to.</rule>
...
<rule id="doc-rides-the-first-commit">A `committed` TDD is part of the branch's FIRST commit, never its own commit and never a trailing "update the design doc" commit. First write, before the PR exists: `~/.cursor/skills/tdd/scripts/tdd-stamp.sh <repo-dir> src/docs/<slug>.md --fold` stamps it with the fingerprint of the code tree it describes and commits it as a fixup of the first commit (folded at once when review-mode allows). Every later revision: commit the edit as a fixup of the first commit with a body (`~/.cursor/skills/lint-commit.sh --fixup $(git rev-list --reverse origin/<base>..HEAD | head -1) --for human|auto -m "<what the doc now says and why>" src/docs/<slug>.md`; `human` when a reviewer or operator asked for the revision, `auto` when it only tracks code the run changed); the push (`pr-finalize-fixups.sh`) re-stamps an edited doc against the pushed tree and condenses it with the other fixups, so no run stamps by hand between rounds. Revise the doc AFTER the turn's last code change: the Complete gate (`require-tdd-current.sh`) compares the stamp to HEAD's code tree, and the push re-stamps only a doc whose text changed, so a stale doc nobody re-read blocks Complete until it is re-read. A reviewer who opens the first commit sees the design with the code; the branch carries no doc-revision history (phase evolution lives in the doc's own `## Phase history`, per `current-state-body-phases-in-one-section`).</rule>
...
## Contents
<numbered list of anchor links, one per ## section, plus a nested
 entry per repo subsection of Design overview (clickable-everything)>

## 1. Problem
## 2. Prior art (why existing approach X is not the answer)
## 3. Goals and non-goals
## 4. Design overview        <- repo table + cross-repo diagram live here
### 4.1 Detailed design: <repo A>    <- per-repo internals go to #### 4.1.1, …
### 4.2 Detailed design: <repo B>    <- one ### per additional repo
## 5. Testing                <- numbered cases; enumerable and checkable
## 6. Phase history          <- the ONLY place evolution lives: one ### per phase
...
## 7. Decisions              <- one ### per decision, per decisions-with-alternatives
## 8. Glossary              <- one ### per term: expansion, what it does HERE,
                            <- and a source link (glossary-for-jargon)
## 9. References
## 10. Post-implementation retrospective   <- added later, per post-impl-retro
```
</template>
...
**`committed`:** write the doc to `src/docs/<slug>.md` in the PR's worktree (`mkdir -p src/docs`), fold it into the branch's first commit per `doc-rides-the-first-commit` (`tdd-stamp.sh <repo-dir> src/docs/<slug>.md --fold`), and push to the PR branch (`--force-with-lease`, since the fold rewrote history; in review-mode `preserve` this needs the operator's rewrite approval or waits for the next sanctioned rewrite). Cite the blob URL on that branch. Then link the doc from EVERY PR of the task, as the body's FIRST section so the reviewer sees the design before the description (`gh pr edit <num> --body-file` in place, existing template preserved below it):
