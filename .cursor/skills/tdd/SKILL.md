...
<rule id="doc-rides-the-first-commit">A `committed` TDD is part of the branch's FIRST commit, never its own commit and never a trailing "update the design doc" commit: after writing or revising the file, stamp it (`~/.cursor/skills/tdd/scripts/tdd-stamp.sh <repo-dir> src/docs/<slug>.md`, which records the fingerprint of the code tree it describes) and commit it as a fixup into the first commit (`~/.cursor/skills/lint-commit.sh --fixup $(git rev-list --reverse origin/<base>..HEAD | head -1) src/docs/<slug>.md`; lint-commit folds it there when review-mode allows, otherwise it waits as a fixup! for the next sanctioned squash). Stamp AFTER the turn's last code commit: the Complete gate (`require-tdd-current.sh`) compares the stamp to HEAD's code tree, so a doc stamped before a later code change blocks until it is re-read and re-stamped. A reviewer who opens the first commit sees the design with the code; the branch carries no doc-revision history (phase evolution lives in the doc's own `## Phase history`, per `current-state-body-phases-in-one-section`).</rule>
...
**`committed`:** write the doc to `src/docs/<slug>.md` in the PR's worktree (`mkdir -p src/docs`), stamp it and fold it into the branch's first commit per `doc-rides-the-first-commit`, and push to the PR branch (`--force-with-lease`, since the fold rewrote history; in review-mode `preserve` this needs the operator's rewrite approval or waits for the next sanctioned rewrite). Cite the blob URL on that branch. Then link the doc from EVERY PR of the task, as the body's FIRST section so the reviewer sees the design before the description (`gh pr edit <num> --body-file` in place, existing template preserved below it):
