...
<rule id="auth-link-relay">Publish auth links follow pr-land `npm-publish-auth`, restated here because every publish phase depends on it: relay each `AUTH_URL` the MOMENT it prints, as a bare clickable URL in the MESSAGE text AND inside the push notification body (a push saying "see chat" is dead weight on a phone). A fresh link supersedes all prior ones; say so rather than re-listing. An `AUTH_DONE login` line (the operator's tap landed) is relayed the same way, noting that the publish link follows after the repo's `prepack`. Watch the script's log file with a polling loop, not a batched event stream: HOLD that loop from script start until `PUBLISHED`/`FAILED` (including while you compose the relay message), and RE-READ the log for the newest line immediately before every message carrying a url. Never send a url quoted from your own earlier message; links expire on roughly a 5-minute cadence and the script remints. A fresh link after the operator completed the previous one, or any `FAILED` line, means npm rejected the request after auth: read the kept capture (the script prints its work dir) before relaying anything further.</rule>
