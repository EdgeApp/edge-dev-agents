...
- **Driver:** `~/.config/agent-watcher/` watches Asana tasks and spawns Claude/Codex agent sessions into per-task git worktrees, with pooled iOS sims, resource/OOM watchdog (`session-watchdog.js`), and slot management. Asana custom fields (`agent_status`, `blocked`) are the run-state channel — not comments.
